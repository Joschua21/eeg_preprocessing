...
class TimeSeriesStateViewer(object):
    ...
    def __init__(self, data_axis, state_axis,
        ...
        self._callback_ids.append(self.figure.canvas.mpl_connect('button_press_event'  , self._on_button_press))
        self._callback_ids.append(self.figure.canvas.mpl_connect('button_release_event', self._on_button_release))
        self._callback_ids.append(self.figure.canvas.mpl_connect('axes_leave_event'    , self._on_axes_leave))
        self._callback_ids.append(self.figure.canvas.mpl_connect('key_press_event'     , self._on_key_press))
        #  self.figure.canvas.mpl_connect('key_release_event'   , self._on_key_release)
        self._callback_ids.append(self.figure.canvas.mpl_connect('motion_notify_event' , self._on_motion))
        self._callback_ids.append(self.figure.canvas.mpl_connect('close_event'         , self._on_close))
    ...
    def _display_help(self):
        ...
        Basic navigation:
        -----------------
        left       -- move backward by one view length (`default_view_length`)
        right      -- move forward  by one view length
        ctrl+left  -- move backward by one epoch length (`default_selection_length`)
        ctrl+right -- move forward  by one epoch length
    ...
    def _update_selection(self, selection_lower_bound, selection_upper_bound):
        # FRANCESCA UPDATED TO LATEST VERSION OF THE CODE ON GIT WHERE ISSUE WAS RESOLVED, MANUALLY. ON 30/08/2024:
