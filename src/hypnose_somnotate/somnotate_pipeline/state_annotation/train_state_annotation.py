...
if __name__ == '__main__':
    ...
    for ii, (idx, dataset) in enumerate(datasets.iterrows()):
        ...
        if len(signal_array) != len(state_vector):
            min_len = min(len(signal_array), len(state_vector))
            print(
                "Length mismatch; trimming to {} (signals: {}, states: {}).".format(
                    min_len, len(signal_array), len(state_vector)
                )
            )
            signal_array = signal_array[:min_len]
            state_vector = state_vector[:min_len]
